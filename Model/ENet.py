...
class RegularBottleneck(nn.Module):
    ...
    def __init__(self, in_out_channels, internal_ratio=4, kernel_size=3, padding=0, dilation=1, asymmetric=False,
                 dropout_prob=0, bias=False, relu=True):
        super(RegularBottleneck, self).__init__()
        # Check in the internal_scale parameter is within the expected range
        # [1, channels]
        if internal_ratio <= 1 or internal_ratio > in_out_channels:
            raise RuntimeError("Value out of range. Expected value in the "
                               "interval [1, {0}], got internal_scale={1}."
                               .format(in_out_channels, internal_ratio))
        ...
        self.ext_conv3 = nn.Sequential(
            nn.Conv2d(
                ...
                kernel_size=1,
                stride=1,
                bias=bias), nn.BatchNorm2d(in_out_channels), activation())
...
class DownsamplingBottleneck(nn.Module):
    ...
    def forward(self, x):
        ...
        # why：主分支是最大池化不修改维度，而旁分支经过2x2,3x3,1x1后输出维度和输入维度不同。因此需要对主分支的维度进行填充。
...
class ENet(nn.Module):
    def __init__(self, num_classes=1, encoder_relu=False, decoder_relu=True):
        ...
        self.transposed_conv = nn.ConvTranspose2d(16, num_classes, kernel_size=3, padding=1, stride=2,output_padding=1,
                                                  bias=False)
    ...
    def forward(self, x):
        ...
        # Stage 5 - Decoder
        x = self.upsample5_0(x, max_indices1_0, stage1_input_size)
        x = self.regular5_1(x)
